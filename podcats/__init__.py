...
class Channel(object):
    ...
    def __init__(self, root_dir, root_url, host, port, title, link,
                 debug=False, cert=None, key=None):
...
def serve(channel):
    ...
    context = None
    if channel.cert is not None:
        assert channel.cert is not None and channel.key is not None, 'Both cert and key should be set for HTTPS'

        context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        context.load_cert_chain(channel.cert, channel.key)
...
parser.add_argument('--cert', help='Cert file for HTTPS')
parser.add_argument('--key', help='Key file for HTTPS')
